...
from pods_logs import PodsLogs
...
class Report:
    """
        ...
        pods_list()
            ...
            all namespaces
        pods_logs()
            Returns the pods previous logs on specific pod.
    ...
    def __init__(self, namespace=False, period=2, ignore_warnings=False,
                 headerless=False, pods_list=[], cluster_info=[]):
        """
            Parameters
            ----------
            namespace : str
    ...
    def get_restarting_containers_report(self):
        ...
        cluster_name = self.cluster_info.get()
                ...
                if pod.status.phase.lower() == 'running':
                    for container in pod.status.container_statuses:
                        if container.restart_count > 0:
                            ...
                            container_time = working_date.astimezone(timezone("Europe/Madrid"))
...
if __name__ == "__main__":
    try:
        ...
        cluster_info = ClusterInfo(auth_method='local', namespace=args.namespace)
        report = Report(args.namespace, args.period, args.ignore_warnings,
                        args.remove_headers, pods_list, cluster_info)
